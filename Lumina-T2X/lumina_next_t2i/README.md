...
> TokenLight-Lumina 是本目录中的独立重光照训练路径，入口与完整流程见
> [`TOKENLIGHT_GUIDE.md`](TOKENLIGHT_GUIDE.md)。它使用公开 Next-DiT 2B 作为
> backbone substitution，不代表未公开 TokenLight 权重的精确复现；原始文本生成路径保持不变。
